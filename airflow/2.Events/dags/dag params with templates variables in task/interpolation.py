...
Python string interpolation to replace variables manually.


Usage:

    Manually replacing the string {{ ds }} with the data context["ds"], 
    which is the value of the date variable provided by Airflow at runtime.
...
@dag(
    dag_id="interpolation_of_params",
    schedule="@daily",
...
def task_flow():
    ...
    def _calculate_stats(**context):
        ...
        input_path = context["params"]["input_path"].replace("{{ ds }}", context["ds"])
        output_path = context["params"]["output_path"].replace(
            "{{ ds }}", context["ds"]
        )
